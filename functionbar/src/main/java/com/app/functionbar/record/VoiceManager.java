                ...
                switch (msg.what) {
                    ...
                    case MSG_TIME_INTERVAL:
                        ...
                        //播放
                            ...
                            //回调播放进度
                            if (voicePlayCallBack != null) {
                                voicePlayCallBack.playDoing(current / 1000, String.format("%02d:%02d",
                                        ts.mSpanMinute, ts.mSpanSecond));
//                                ToastUtil.show(mContext,String.format("%02d:%02d",
//                                        ts.mSpanMinute, ts.mSpanSecond));
                            }
                            mHandler.sendEmptyMessageDelayed(MSG_TIME_INTERVAL, 1000);
                        }
    ...
    public void stopVoiceRecord() {
        try {
            ...
            if (mRecTimeSum < 1) {
                ...
                voiceRecordCallBack.recException();
            } else {
                ...
                if (file != null && file.length() > 0) {
                    ...
                    //完成录音
                    if (voiceRecordCallBack != null) {
                        voiceRecordCallBack.recFinish(mRecTimeSum, String.format("%02d:%02d",
                                ts.mSpanMinute, ts.mSpanSecond), file.getAbsolutePath());
                    }
                }
            }
        ...
        if (mSBPlayProgress != null) {
            mSBPlayProgress.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
                ...
                public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                    ...
                    //播放进度
                    if (voicePlayCallBack != null) {
                        voicePlayCallBack.playDoing(progress / 1000, String.format("%02d:%02d",
                                ts.mSpanMinute, ts.mSpanSecond));
                    }
                }
     ...
     * @param init
     */
    private void startPlay(final boolean init) {
        try {
            stopRecorder(mMediaRecorder, true);
            mMediaRecorder = null;
     ...
     * 准备好后调用
     */
    private void onPrepare(boolean init) {
        ...
        if (init) {
            ...
            seektoMedia(mMediaPlayer, mSBPlayProgress!=null?mSBPlayProgress.getProgress():0);
        }
    ...
    public boolean isStoping() {
        return mDeviceState == MEDIA_STATE_PLAY_PAUSE;
    }
